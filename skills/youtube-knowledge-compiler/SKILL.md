...
description: >
  ...
  whenever Zaeem pastes a video transcript, shares a YouTube URL, says "I found a video on",
  ...
  into skills, memory, CLAUDE.md, or the knowledge archive. Also trigger when Zaeem says
...
Read the full transcript/content. Extract ONLY things that could improve Zaeem's workflows.
...
- **Why it matters**: How this connects to Zaeem's current stack (Rafiq, Mirath, Orchestrator, Claude Code, skills library, etc.)
...
- **Islamic finance domain**: screening methodologies, fiqh rulings, AAOIFI standards
...
|-------|-------------|--------|
| **Memory edit** | Persistent preference, tool choice, or workflow pattern Zaeem should benefit from across all sessions | Use `memory_user_edits` tool to add |
| **Skill update** | Technique that improves an existing skill's instructions or output quality | Identify the skill, draft the edit, confirm with Zaeem |
| **New skill** | Pattern substantial enough to warrant its own reusable workflow | Draft SKILL.md, confirm with Zaeem |
| **CLAUDE.md patch** | Claude Code-specific instruction, hook, or agent behavior change | Draft the addition for Zaeem to add to his repo |
...
3. **New skills** — draft the SKILL.md, confirm scope with Zaeem
...
When Zaeem says "run a health check" or "what haven't we routed":
...
4. Suggest content gaps: topics Zaeem's stack would benefit from that haven't been covered yet

## Quality Filters

- **No fluff**: Skip motivational content, hype, or restated obvious advice
- **Testable claims only**: If a video says "X is better", ask: can we verify this in Zaeem's codebase?
...
- **Domain accuracy**: For Islamic finance claims, hold to the same rigor as all Rafiq work — madhabs for fiqh, methodologies (AAOIFI/DJIM/S&P/MSCI) for screening, never conflate
