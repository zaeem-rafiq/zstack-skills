...
### Proof-of-Success (machine-checkable)
One executable check per acceptance criterion above. The agent runs each and prints the
result; the loop marks the issue done ONLY when every row passes. A criterion with no
runnable command is not a contract — it's a wish.

| Criterion | Command | Expected exit | Expected stdout signature |
|---|---|---|---|
| 1 | `[exact command, e.g. npm test -- search.spec.ts]` | `0` | `[grep target, e.g. "3 passed"]` |
| 2 | `[command]` | `0` | `[signature]` |
| 3 | `[command]` | `0` | `[signature]` |

### Bounds
- **Max turns:** [N] · **Max wall-clock:** [e.g. 30m] · **Cost ceiling:** [e.g. $5.00]
- **On exceed → stop. Do NOT retry. Invoke Failure Handoff below.**

### Protected Paths
Files/dirs the agent MUST NOT create, modify, or delete:
- `[path or glob, e.g. infra/**, .github/workflows/**, db/migrations/**]`
- (If a change here seems required, stop and invoke Failure Handoff — do not edit.)
- Use `none` only with a one-line justification.

### Failure Handoff
- **Blocker doc:** write to `[path, e.g. ./blockers/issue-[id].md]` — what failed, the last
  command + output, and which bound or protected path was hit.
- **Status label:** set the issue to `[e.g. Blocked]`.
- **Notify:** post to `[e.g. #eng-orchestrator]` with the issue id and blocker-doc path.
...
- **A Proof-of-Success references a test or command that does not exist yet.** A contract
  that points at `npm test path/to/spec` when that spec has not been written — or at a
  script, flag, or binary not in the repo — is not machine-checkable; the agent fails at
  runtime with "file not found," not at a real assertion. Either create the test as part of
  this issue (say so in Technical Notes) or point the command at something that exists on a
  clean checkout. Never ship a Proof-of-Success that cannot run.
...
- [ ] **Every acceptance criterion has a Proof-of-Success row.** Each maps to one
  executable command with an expected exit code and a stdout signature the agent can grep.
  No criterion is left without a machine-checkable command.
- [ ] **Every issue declares Bounds.** Max turns, max wall-clock, and cost ceiling are all
  set, plus the explicit "on exceed → stop, do not retry, invoke Failure Handoff" rule.
- [ ] **Every issue lists Protected Paths.** Files/dirs the agent must not modify are named
  (or the field states "none" with justification).
- [ ] **Every issue defines Failure Handoff.** Blocker-doc path, status label, and notify
  target are all specified so a stuck agent has a deterministic exit.
