...
- If you need teammates that talk to each other instead of just reporting back, use Claude Code Agent Teams and reference these personas as teammate types.

**Persona resolution.** Use `code-reviewer`, `security-auditor`, and `test-engineer` from `.claude/agents/` or `~/.claude/agents/`. Project-level definitions take precedence over user-level definitions; zstack sync installs the user-level defaults.
